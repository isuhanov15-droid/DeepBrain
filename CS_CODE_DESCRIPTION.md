...
- `BrainLifeStateSubscribe`, `BrainLifeState`, `BrainLifeEpisodeAppend` — подписка и доставка состояния жизненного цикла v0.2 (эпизоды зарезервированы).
...
- флаги подписок: `_wantsLogs`, `_wantsState`, `_wantsTrace`, `_wantsLifeState`.
**RunAsync:**
...
**HandleAsync:**
- `ping` → `pong`.
- `logs.subscribe`, `brain.state.subscribe`, `trace.subscribe`, `brain.life.state.subscribe` → включают флаги и логируют.
...
**Отправка:**
- `SendLogAsync`, `SendStateAsync`, `SendTraceAsync`, `SendLifeStateAsync`.
...
12) trace по стадиям (homeostasis/instincts/affect/action/reward).
...
**Connect:** ping + подписки logs/state/trace/life.
