...
Назначение: долговременная эпизодическая память v1.1.
Поведение:
- сохраняет компактный итог завершённого эпизода в ограниченный JSONL;
- загружает память после перезапуска и пропускает повреждённые строки;
- ищет похожие эпизоды по сценарию, настроению и гомеостатическому состоянию;
- учитывает значимость и давность воспоминаний;
- возвращает ограниченные положительные или отрицательные смещения оценок действий;
- автоматически уплотняет хранилище при превышении лимита.
...
- v1.1: долговременная память влияет только на уже созданные и разрешённые кандидаты.
