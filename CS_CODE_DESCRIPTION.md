...
v1.1 добавляет постоянную эпизодическую память, переживающую перезапуск Host, контекстный recall и ограниченное влияние воспоминаний на выбор действий.
v1.2 добавляет когнитивную Memory v2: дедупликацию, консолидацию, забывание, индексированный recall, устойчивый опыт сценариев и объяснение результата действий.
v1.3 добавляет наблюдательный LLM Cortex со строгим JSON-контрактом, компактным семантическим кадром и измерением latency/tokens/размера данных, а также защищённый HTTP/SSE API с универсальным интерфейсом будущих MQTT, Home Assistant и ROS2-адаптеров.
...
Назначение: кратковременная эпизодическая память в RAM.
Поведение:
...
### src/DeepBrain.Host/BrainLife/PersistentEpisodicMemory.cs
Назначение: когнитивная долговременная память v2.
Поведение:
- сохраняет итог завершённого эпизода в ограниченный совместимый JSONL;
- загружает память после перезапуска и пропускает повреждённые строки;
- защищает от дубликатов, консолидирует похожие исходы и усиливает повторяющийся опыт;
- забывает старые малозначимые записи, сохраняя критический и повторный опыт;
- индексирует сценарий, настроение и гомеостатическое состояние для ограниченного recall;
- агрегирует устойчивый опыт по сценариям и результатам отдельных действий;
- объясняет полезные и вредные действия и возвращает ограниченные смещения их оценок.
...
- v1.2: когнитивная память учитывает результат действия и устойчивый опыт сценария, но по-прежнему влияет только на созданные и разрешённые кандидаты.
